...
if defined _coreed (
    ...
    for /f "tokens=4" %%a in ('dism /online /english /Get-TargetEditions ^| findstr /i /c:"Target Edition : "') do (
        findstr /i /r /c:"^^%%a~.*" "!work!\bin\keys.ini" %_nul% && (
            echo %%a | findstr /i /c:"Core" /c:"Education" /c:"Professional" /c:"ServerRdsh" %_nul% && (
                set "%%a=1"
                echo %%a
            )
        )
  ...
  for %%# in (
    ...
    for /f "tokens=4" %%a in ('dism /online /english /Get-TargetEditions ^| findstr /i /c:"Target Edition : "') do (
        ...
        findstr /i /r /c:"^^%%a~.*" "!work!\bin\buildkeys.ini" %_nul% && (
            set "%%a=1"
            echo %%a
        )
    )
...
for /f "usebackq eol=; delims=" %%# in ("!work!\bin\buildkeys.ini") do for /f "tokens=1-4 delims=~" %%A in ("%%#") do if /i [%_edchc%]==[%%A] if /i %osbuild% GEQ %%B (
    set "swkey=%%C"
    set "swkeytype=%%D"
)
...
if defined _coreed (
changepk /ProductKey "%swkey%" %_nul%
) else (
%_ipk% "!batw!" "%swkey%" %_nul%
)
if not [%errorlevel%]==[0] (
call :colortxt %red% "Unsuccessful" /n
) else (
